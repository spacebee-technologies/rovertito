...
/*=====================[Variables]================================*/
    ...
    volatile static APP_STATES state = APP_STATE_MCAN_USER_INPUT; //Variable para guardar el estado de la aplicación
...
/*========================================================================
  ...
  Descripcion: Establece la configuración de RAM de mensajes can. Previamente se debe haber llamado a MCAN1_Initialize para la instancia de MCAN asociada.
  Parametro de entrada:
                        uint8_t *msgRAMConfigBaseAddress: Puntero a la dirección base del búfer asignada a la aplicación. La aplicación debe asignar el búfer desde la memoria contigua no almacenada en caché y el tamaño del búfer debe ser MCAN1_MESSAGE_RAM_CONFIG_SIZE
...
void mcan_fd_interrupt_config(uint8_t *msgRAMConfigBaseAddress){
    MCAN1_MessageRAMConfigSet(msgRAMConfigBaseAddress);     //Establece configuración de RAM de mensajes
}
...
/*========================================================================
  ...
  ========================================================================*/
bool mcan_fd_interrupt_enviar(uint32_t messageID , uint8_t *message, MCAN_MODE MCAN_MODE, MCAN_MSG_TX_ATTRIBUTE MCAN_MSG_ATTR_TX){
    if (state == APP_STATE_MCAN_USER_INPUT)                                                     //Si se esta esperando al usuario para enviar o recibir mensaje
        {           
            ...
            bool Resultado = MCAN1_MessageTransmit(messageID, sizeof(message), message, MCAN_MODE, MCAN_MSG_ATTR_TX); //Transmite un mensaje al bus CAN. Previamente se debe haber llamado a MCAN1_Initialize para la instancia de MCAN asociada.
...
/*========================================================================
  ...
  Parametro de entrada:
                        ...
                        MCAN_MSG_ATTR_RX:           Trama de datos o trama remota usando Tx FIFO o Tx Buffer. Mensaje para ser leído desde Rx FIFO0 o Rx FIFO1 o Rx Buffer
...
bool mcan_fd_interrupt_recibir(uint32_t *rx_messageID, uint8_t *rx_message, uint8_t *rx_messageLength, uint16_t *timestamp, MCAN_MSG_RX_ATTRIBUTE MCAN_MSG_ATTR_RX, MCAN_MSG_RX_FRAME_ATTRIBUTE *msgFrameAttr){  
    if (state == APP_STATE_MCAN_USER_INPUT)                                                                     //Si se esta esperando al usuario para enviar o recibir mensaje
        {
            MCAN1_RxCallbackRegister( APP_MCAN_Callback, (uintptr_t)APP_STATE_MCAN_RECEIVE, MCAN_MSG_ATTR_RX); //Establece el puntero a la función (y su contexto) que se llamará cuando ocurran los eventos de transferencia del MCAN dado. Previamente se debe haber llamado a MCAN1_Initialize para la instancia de MCAN asociada.
            state = APP_STATE_MCAN_IDLE;                                                                       //Establezco el estado en estado mcan inactivo
            uint8_t rx_message_size=sizeof(rx_message);                                                        //Obtengo tamaño del mensaje
            memset(rx_message, 0x00, rx_message_size);                                                         //memset(void *str, int c, size_t n) copia el carácter c (un carácter sin signo) en los primeros n caracteres de la cadena a la que apunta el argumento str .
